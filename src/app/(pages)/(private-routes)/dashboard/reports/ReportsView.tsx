...
function ReportBody({
  ...
  if (loading) {
    return <p className="text-xs text-foreground-muted">در حال بارگذاری…</p>;
  }
  if (error) {
    return (
...
export default function ReportsView() {
  ...
  const onExport = async () => {
    try {
      ...
      setToast({ type: "success", message: "خروجی CSV آماده شد." });
    } catch (err) {
      setToast({
        type: "error",
        message: getApiErrorMessage(err, "خروجی CSV ناموفق بود."),
      });
    }
  };
  ...
  return (
    <DashboardPage>
      <DashboardPageHeader
        title="گزارش‌ها"
        description="مالی، عملیاتی، پرسنل و CRM."
      />

      <DashboardCard className="sticky top-[4.5rem] z-10 space-y-2">
        <div className="grid grid-cols-2 gap-2">
          <DashboardDateField name="reports-from" value={from} onChange={setFrom} label="از" />
          <DashboardDateField name="reports-to" value={to} onChange={setTo} label="تا" />
        </div>
        <DashboardSelect
          value={branchId}
          onChange={(e) =>
      ...
      <DashboardAccordion title="درآمد بر اساس روش پرداخت" defaultOpen>
        <ReportBody loading={byMethod.isLoading} error={byMethod.error}>
          {methodRows.length === 0 ? (
            ...
            methodRows.map((row, i) => (
              <Row
                ...
                label={row.methodName || paymentMethodLabel(row.paymentMethod)}
                value={`${formatMoneyOrDash(row.collected ?? row.amount)}${
                  row.refunds != null
                    ? ` / استرداد ${formatMoneyOrDash(row.refunds)}`
                    : ""
                }`}
              />
            ))
      ...
      <DashboardAccordion title="درآمد بر اساس خدمت">
        <ReportBody loading={byService.isLoading} error={byService.error}>
      ...
      <DashboardAccordion title="درآمد بر اساس شعبه">
        <ReportBody loading={byBranch.isLoading} error={byBranch.error}>
          {branchRows.length === 0 ? (
            <p className="text-xs text-foreground-muted">داده‌ای نیست.</p>
          ) : (
      ...
      <DashboardAccordion title="سری روزانه">
        <ReportBody loading={byDay.isLoading} error={byDay.error}>
          {dayRows.length === 0 ? (
            <p className="text-xs text-foreground-muted">داده‌ای نیست.</p>
          ) : (
      ...
      <DashboardAccordion title="مطالبات و کسورات">
        <ReportBody loading={outstanding.isLoading} error={outstanding.error}>
          {outstandingData ? (
            <>
              <Row
                label="مانده فاکتور"
      ...
      <DashboardAccordion title="قیف نوبت">
        <ReportBody loading={funnel.isLoading} error={funnel.error}>
          ...
          {sourceRows.length === 0 ? (
            ...
            sourceRows.map((row, i) => (
              <Row
                ...
                label={row.name || appointmentSourceLabel(row.source)}
                value={String(row.count ?? 0)}
              />
            ))
          )}
      ...
      <DashboardAccordion title="عملکرد پرسنل">
        <ReportBody loading={staff.isLoading} error={staff.error}>
          {staffRows.length === 0 ? (
            <p className="text-xs text-foreground-muted">داده‌ای نیست.</p>
          ) : (
            staffRows.map((row, i) => (
      ...
      <DashboardAccordion title="ساعات اوج">
        <ReportBody loading={peak.isLoading} error={peak.error}>
      ...
      <DashboardAccordion title="نرخ پرشدن">
        <ReportBody loading={fill.isLoading} error={fill.error}>
          {fillData ? (
            <>
              <Row
              ...
              />
              <Row label="بافر" value={String(fillData.bufferMinutes ?? "—")} />
              <Row label="نرخ پرشدن" value={formatRate(fillData.fillRate)} />
            </>
          ) : (
            <p className="text-xs text-foreground-muted">داده‌ای نیست.</p>
          )}
      ...
      <DashboardAccordion title="خلاصه CRM">
        <ReportBody loading={crmSummary.isLoading} error={crmSummary.error}>
          {crm ? (
            <>
      ...
      <DashboardAccordion title="مشتریان برتر">
        <ReportBody loading={topCustomers.isLoading} error={topCustomers.error}>
          {topRows.length === 0 ? (
            <p className="text-xs text-foreground-muted">داده‌ای نیست.</p>
          ) : (
            topRows.map((row, i) => (
