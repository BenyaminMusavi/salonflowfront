...
- Online booking wizard (`/salons/[id]/book`, 5 steps): 1 services (branch picker on the same step only for multi-branch salons) → 2 staff from `GET staff-profiles/by-salon/{id}/for-services` or «اولین نوبت» (`GET /api/booking/first-available` once → prefills date/time/staff; on error/404 fall back to all staff's slots and take staff from the picked slot) → 3 dates (`available-dates`, `staffPublicId` when known) with that day's `GET /api/booking/slots` underneath → 4 `calculate-price` with the resolved staff → 5 confirm + `POST /api/booking/create`. Draft in sessionStorage (`bookDraft.ts`, bump `DRAFT_VERSION` when steps change). Exit salon JWT context before create.
